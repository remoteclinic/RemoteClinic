...
$input_email=mysql_escape_string($input_email);
$input_email=mysql_real_escape_string($input_email);

$input_password=stripslashes($input_password);
$input_password=mysql_escape_string($input_password);
...
if($rec=mysql_fetch_array(mysql_query("SELECT * FROM p_staff_dir WHERE userid='$input_email'"))){
...
if($rec=mysql_fetch_array(mysql_query("SELECT * FROM p_staff_dir WHERE userid='$input_email' AND passkey='$input_password'"))){
...
mysql_query("update p_staff_dir set last_update='$current_time' where id=$_SESSION[id]																																																			") or die(mysql_error());

write_log("$_SESSION[id]","LoggedIn","staff","10");

     print "<script>";
	        print " self.location='../desktop/';"; 
          print "</script>";
